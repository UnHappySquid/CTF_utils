...
// For simplicity, this class will assume BITMAPINFOHEADER_SIZE for BodyInfoHeader
struct BmpFile {
  struct FileHeader {
  ...
  BmpFile(istream& ifs) {
    ...
    if (tmp_size != BITMAPCOREHEADER_SIZE &&
      ...
      cout << string(90, '-') << endl;
      cout << red_coat("[WARNING]") << " Body info header size is unconventional" << endl;
      cout << red_coat("[WARNING]") << " body info size is : " << tmp_size << endl;
      cout << red_coat("[WARNING]") << " refer to " << size_wiki_link<< " for more info." << endl;
      cout << string(90, '-') << endl;
    }
  ...
  private:
  string red_coat(string s) {
    return "\033[1;31m" + s + "\033[1;0m";
  }
...
int main(int argn, char** args) {
  if (sizeof(unsigned short int) != 2 ||
      ...
      sizeof(int) != 4) {
    cout << "The type sizes don't comply with the programs assumptions about them." << endl;
    exit(2);
  }
  if (argn != 2) {
    usage(args[0]);
